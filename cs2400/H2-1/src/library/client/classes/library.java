...
import library.bag.classes.LinkedBag;
class library{
	LinkedBag<BookRecord> bookDB = new LinkedBag<>();
	...
	public void print(){//list the library
		System.out.println(bookDB.printBag());
		/*for(int i=0;i<this.noRecords;++i){
			System.out.println(this.books[i].toString());
		}*/
	}
	...
	public static void main(String []args){//instantiate the library
		...
		File myFile = new File(scanner.next());
		...
		try {
			...
			while(scan.hasNextLine()){
				...
				if(!myLib.bookDB.contains(aRecord)){
					myLib.bookDB.add(aRecord);
					myLib.noRecords++;
				}
